...
/**
 * Class IntegerNet_Varnish_Model_Index_Import_Enterprise
 */
    ...
    public function importCategoryUrl($storeId)
    {
        ...
        if (is_numeric($storeId)
            ...
            foreach ($index->getReadConnection()->fetchPairs($select) as $categoryId => $requestPath) {
                $importCount += $index->indexUrl(Mage::app()->getStore($storeId)->getBaseUrl('web', false) . $requestPath, 'catalog/category/view', -1);
            }
        }
    ...
    public function importProductUrl($storeId)
    {
        ...
        if (is_numeric($storeId)
            ...
            foreach ($index->getReadConnection()->fetchPairs($select) as $productId => $requestPath) {
                $importCount += $index->indexUrl(Mage::app()->getStore($storeId)->getBaseUrl('web', false) . $requestPath, 'catalog/product/view', -1);
            }
